...
elseif ($_GET['module']=='kontrak'){
      echo "<span class='judulhead'><b>Data Kontrak</b></span>";
  }
  elseif ($_GET['module']=='jurusan'){
      echo "<span class='judulhead'><b>Managemen Jurusan</b></span>";
  }
...
elseif($_GET['module']=='detailpengajar'){
        $stmt = $db->prepare("SELECT * FROM pengajar WHERE id_pengajar = ?");
...
elseif($_GET['module']=='daftarsiswa'){
        $stmt = $db->prepare("SELECT * FROM siswa WHERE id_kelas = ? LIMIT 1");
        $stmt->execute([$_GET['id']]);
        $s = $stmt->fetch(PDO::FETCH_ASSOC);
...
elseif($_GET['module']=='jurusan'){
	echo "<span class='judulhead'><b>Manajeman Jurusan</b></span>";
}
