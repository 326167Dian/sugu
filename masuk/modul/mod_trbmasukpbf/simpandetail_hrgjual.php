...
// input ditampilkan dalam format rupiah, jadi titik ribuan harus dibuang dulu
...
try {
    ...
    if ($trbmasuk->rowCount() > 0) {
        ...
        $db->prepare("UPDATE trbmasuk_detail SET hrgjual_dtrbmasuk = ? WHERE kd_barang = ? AND kd_trbmasuk = ?")
            ->execute([$hrgjual_dtrbmasuk, $kd_barang, $kd_trbmasuk]);

        $hrgjual_barang = round($hrgjual_dtrbmasuk);
        $db->prepare("UPDATE barang SET hrgjual_barang = ? WHERE id_barang = ?")->execute([$hrgjual_barang, $detail['id_barang']]);
        ...
        $db->prepare("INSERT INTO trbmasuk_detail(
                                    ...
                                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)")
            ->execute([$kd_trbmasuk, $kd_orders, $odt['id_barang'], $odt['kd_barang'], $odt['nmbrg_dtrbmasuk'], $qty_dtrbmasuk, $qtygrosir_dtrbmasuk, $odt['sat_dtrbmasuk'], $odt['satgrosir_dtrbmasuk'], $odt['konversi'], $rst['hna'], $odt['diskon'], $harga_satuan, $hrgjual_dtrbmasuk, $total_harga, $odt['no_batch'], $odt['exp_date'], $waktu]);
