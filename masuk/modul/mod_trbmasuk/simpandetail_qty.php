...
try {
    ...
    $stmt_detail = $db->prepare("SELECT id_dtrbmasuk, id_barang, kd_barang, kd_trbmasuk, qty_dtrbmasuk, hrgsat_dtrbmasuk, no_batch
    ...
    $hrgttl_dtrbmasuk = round($qty_dtrbmasuk * (float) $detail['hrgsat_dtrbmasuk'], 0);

    $stmt_update_detail = $db->prepare("UPDATE trbmasuk_detail
                                        SET qty_dtrbmasuk = ?,
                                            hrgttl_dtrbmasuk = ?
                                        WHERE id_dtrbmasuk = ?");
    $stmt_update_detail->execute([$qty_dtrbmasuk, $hrgttl_dtrbmasuk, $id_dtrbmasuk]);
