...
try {
    ...
    if ($trbmasuk->rowCount() > 0) {
        ...
        $db->prepare("UPDATE trbmasuk_detail SET exp_date = ? WHERE id_dtrbmasuk = ?")
            ->execute([$exp_date, $detail['id_dtrbmasuk']]);

        if (!empty($detail['no_batch'])) {
            $db->prepare("UPDATE batch SET exp_date = ?
                            WHERE kd_transaksi = ? AND kd_barang = ? AND no_batch = ? AND status = 'masuk'")
                ->execute([$exp_date, $kd_trbmasuk, $kd_barang, $detail['no_batch']]);
        }
        ...
        $db->prepare("INSERT INTO trbmasuk_detail (kd_trbmasuk, kd_orders, id_barang, kd_barang, nmbrg_dtrbmasuk, qty_dtrbmasuk, qty_grosir, sat_dtrbmasuk, satgrosir_dtrbmasuk, konversi, hnasat_dtrbmasuk, diskon, hrgsat_dtrbmasuk, hrgjual_dtrbmasuk, hrgttl_dtrbmasuk, no_batch, exp_date, waktu) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)")
            ->execute([$kd_trbmasuk, $kd_orders, $odt['id_barang'], $odt['kd_barang'], $odt['nmbrg_dtrbmasuk'], $qty_dtrbmasuk, $qtygrosir_dtrbmasuk, $odt['sat_dtrbmasuk'], $odt['satgrosir_dtrbmasuk'], $odt['konversi'], $odt['hrgsat_dtrbmasuk'], $odt['diskon'], $odt['hrgsat_dtrbmasuk'], $odt['hrgjual_dtrbmasuk'], $hrgttl, $odt['no_batch'], $exp_date, $waktu]);
        ...
        if (!empty($odt['no_batch'])) {
            $db->prepare("INSERT INTO batch (tgl_transaksi, no_batch, exp_date, qty, satuan, kd_transaksi, kd_barang, status) VALUES (?, ?, ?, ?, ?, ?, ?, 'masuk')")
                ->execute([$waktu, $odt['no_batch'], $exp_date, $qtygrosir_dtrbmasuk, $odt['sat_dtrbmasuk'], $kd_trbmasuk, $kd_barang]);
        }
    }
