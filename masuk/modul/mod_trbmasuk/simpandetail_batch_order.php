...
try {
    ...
    if ($trbmasuk->rowCount() > 0) {
        ...
        $db->prepare("UPDATE trbmasuk_detail SET no_batch = ? WHERE id_dtrbmasuk = ?")
            ->execute([$no_batch, $detail['id_dtrbmasuk']]);

        if (!empty($no_batch_lama)) {
            $db->prepare("DELETE FROM batch WHERE kd_transaksi = ? AND kd_barang = ? AND no_batch = ? AND status = 'masuk'")
                ->execute([$kd_trbmasuk, $kd_barang, $no_batch_lama]);
        }
        ...
        $db->prepare("INSERT INTO trbmasuk_detail (kd_trbmasuk, kd_orders, id_barang, kd_barang, nmbrg_dtrbmasuk, qty_dtrbmasuk, qty_grosir, sat_dtrbmasuk, satgrosir_dtrbmasuk, konversi, hnasat_dtrbmasuk, diskon, hrgsat_dtrbmasuk, hrgjual_dtrbmasuk, hrgttl_dtrbmasuk, no_batch, exp_date, waktu) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)")
            ->execute([$kd_trbmasuk, $kd_orders, $odt['id_barang'], $odt['kd_barang'], $odt['nmbrg_dtrbmasuk'], $qty_dtrbmasuk, $qtygrosir_dtrbmasuk, $odt['sat_dtrbmasuk'], $odt['satgrosir_dtrbmasuk'], $odt['konversi'], $odt['hrgsat_dtrbmasuk'], $odt['diskon'], $odt['hrgsat_dtrbmasuk'], $odt['hrgjual_dtrbmasuk'], $hrgttl, $no_batch, $odt['exp_date'], $waktu]);
