...
try {
    ...
    if ($trbmasuk->rowCount() > 0) {
        $hrgbelidisc = $detail['hrgsat_dtrbmasuk'] * (1 - ($diskon / 100));
        $hrgttl      = round($hrgbelidisc * $detail['qty_grosir']);

        $db->prepare("UPDATE trbmasuk_detail SET diskon = ?, hrgttl_dtrbmasuk = ? WHERE id_dtrbmasuk = ?")
            ->execute([$diskon, $hrgttl, $detail['id_dtrbmasuk']]);
        ...
        $hrgbelidisc   = $odt['hrgsat_dtrbmasuk'] * (1 - ($diskon / 100));
        ...
        $db->prepare("INSERT INTO trbmasuk_detail (kd_trbmasuk, kd_orders, id_barang, kd_barang, nmbrg_dtrbmasuk, qty_dtrbmasuk, qty_grosir, sat_dtrbmasuk, satgrosir_dtrbmasuk, konversi, hnasat_dtrbmasuk, diskon, hrgsat_dtrbmasuk, hrgjual_dtrbmasuk, hrgttl_dtrbmasuk, no_batch, exp_date, waktu) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)")
            ->execute([$kd_trbmasuk, $kd_orders, $odt['id_barang'], $odt['kd_barang'], $odt['nmbrg_dtrbmasuk'], $qty_dtrbmasuk, $qtygrosir_dtrbmasuk, $odt['sat_dtrbmasuk'], $odt['satgrosir_dtrbmasuk'], $odt['konversi'], $odt['hrgsat_dtrbmasuk'], $diskon, $odt['hrgsat_dtrbmasuk'], $odt['hrgjual_dtrbmasuk'], $hrgttl, $odt['no_batch'], $odt['exp_date'], $waktu]);
