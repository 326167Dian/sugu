        ...
        default:
            ...
            <div class="box box-primary box-solid">
                <div class="box-header with-border">
                    <h3 class="box-title">LAPORAN RE-STOK OPNAME</h3>
                ...
                <div class="box-body">
                    ...
                    <form method="POST" action="?module=laprestokopname&act=laporanbulanan" enctype="multipart/form-data"
...
<div>
     <div>
                    ...
                    <table id="example1" class="table table-bordered table-striped">
                        <thead>
							<tr>
                                ...
                                <td>Jenis SO</td>
                                <td>SO Saat ini</td>
                                <td>Minus SO Sebelum</td>
                                <td>Lebih SO Sebelum</td>
                            </tr>
                        </thead>
                        <tbody>
                            <?php
                            ...
                            $so = $db->prepare("SELECT DISTINCT tgl_stokopname, shift FROM stok_opname2
                            ...
                            while ($r = $so->fetch(PDO::FETCH_ASSOC)) {
                                ...
                                $curent = $db->prepare("select sum(ttl_hrgbrg) as curent from stok_opname2
                                            where shift='$shift' and tgl_stokopname='$r[tgl_stokopname]' ");
                                $curent->execute();
                                $now = $curent->fetch(PDO::FETCH_ASSOC);
                                $kurnow = format_rupiah(round($now['curent'], 0));
        ...
        case "laporanbulanan":
            ...
            $shift = $_POST['shift'];
            $so = $db->prepare("SELECT * FROM stok_opname2 
	JOIN barang ON barang.id_barang = stok_opname2.id_barang 
	LEFT JOIN admin ON admin.id_admin = stok_opname2.id_admin WHERE stok_opname2.shift='$shift' and stok_opname2.tgl_stokopname ='$tgl_awal' ORDER BY barang.nm_barang ASC");
	        $so->execute();

            ?>
            <div class="box box-primary box-solid">
                <div class="box-header with-border">
                    <h3 class="box-title">LAPORAN RE-STOK OPNAME TANGGAL <?= $tgl_awal ?> SD <?= $tgl_akhir ?></h3>
                    <div class="box-tools pull-right">
                        <button class="btn btn-box-tool" data-widget="collapse"><i class="fa fa-minus"></i></button>
                    </div><!-- /.box-tools -->
                </div>
                <?php
                echo "
                <a class='btn  btn-success btn-flat' target='_blank' href='?module=laprestokopname&act=sinkron_min&tgl_awal=$tgl_awal&tgl_akhir=$tgl_akhir&shift=$shift'>SINKRONISASI STOK MINUS</a> 
                <a class='btn  btn-success btn-warning' target='_blank' href='?module=laprestokopname&act=sinkron_plus&tgl_awal=$tgl_awal&tgl_akhir=$tgl_akhir&shift=$shift'>SINKRONISASI STOK PLUS</a>                 
                ";
                ?>
                        ...
                        <table id="example2" class="table table-bordered table-striped">
                            <thead>
                                <tr>
                                    ...
                                    <th style="text-align: center; ">Hasil Sebelum</th>
                            ...
                            <tbody>
                                ...
                                while ($r = $so->fetch(PDO::FETCH_ASSOC)) {
                                    ...
                                    $stokopname_old = $db->prepare("SELECT * FROM stok_opname WHERE shift=? 
                                        AND tgl_stokopname =?
                                        AND kd_barang = ?");
                                    $stokopname_old->execute([$shift, $tgl_awal, $r['kd_barang']]);
                                    $rstok = $stokopname_old->fetch(PDO::FETCH_ASSOC);
                                ...
                                <td style='text-align: center;'>$rstok[selisih]</td>                                
        ...
        case "sinkron_min":
            ...
            $sominus = $db->prepare("SELECT * FROM stok_opname2 
                WHERE ttl_hrgbrg<0 AND shift='$shift' AND tgl_stokopname BETWEEN '$tgl_awal' AND '$tgl_akhir' ");
            $sominus->execute();
            $no1 = 1;
        ...
        case "sinkron_plus":
            ...
            $soplus = $db->prepare("SELECT * FROM stok_opname2 
