...
<table id="example10" class="table table-bordered table-striped">
    ...
    <tbody>
        ...
        $query = $db->prepare("SELECT * FROM barang a 
                                INNER JOIN stok_opname b
                                ON  b.id_barang = a.id_barang
                                WHERE a.jenisobat = ? 
                                AND stok_barang>0
                                AND b.tgl_stokopname = ?
                                AND b.selisih != 0
                                ORDER BY a.nm_barang");
        $query->execute([$jenisobat,$tgl]);

        $no = 1;
        while ($lihat = $query->fetch(PDO::FETCH_ASSOC)) :

            $stokopname = $db->prepare("SELECT * FROM stok_opname2 a 
                                        WHERE a.id_barang = ? 
        ...
        ?>
                ...
                <tr>
                    ...
                    <td class="text-center">
                        <input type="number" min="0" class="form-control text-center" name="stok_fisik_<?= $no ?>" id="stok_fisik_<?= $no ?>" value="<?=$lihat['selisih']?>">
                    </td>
                    <td class="text-center">
