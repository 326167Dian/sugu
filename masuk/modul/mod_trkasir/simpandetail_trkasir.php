            ...
            if ($rowbundle > 0) {
                ...
                while($rbundle = $get_bundle_detail->fetch(PDO::FETCH_ASSOC)){
                    ...
                    $stmt_insert_trkasirdetail = $db->prepare("INSERT INTO trkasir_detail(kd_trkasir,
                                                ...
                                                nm_bundle)
    	    							  VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
                    $stmt_insert_trkasirdetail->execute([$kd_trkasir, $rbundle['id_barang'], $rbundle['kd_barang'], $rbundle['nm_barang'], $qty_dikurangi, $rbundle['sat_barang'], $rbundle['hrgjual_barang'], 
                                        $disc, $resep, $modal, $profit, $no_batch, $exp_date, $ttlharga, $tipe, $komisi, $id_admin, $hdbundle['kd_bundle'], $hdbundle['nm_bundle']]);
